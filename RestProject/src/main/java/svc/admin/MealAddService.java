...
public class MealAddService {
	// 멤버변수
	// 기본 생성자
	// 메서드

	public boolean addMeal(MealKit newMealKit) {
		...
		int intsertNewMealCount = mealKitDAO.insertNewMeal(newMealKit);
		
		/*-(insert,update,delete) 성공하면 commit 
		 *  실패하면 rollback(select 제외)---*/
		boolean isAddMealResult = false;

		if(intsertNewMealCount > 0) {
			isAddMealResult = true;
			commit(con);
		} else {
			rollback(con);
		}
