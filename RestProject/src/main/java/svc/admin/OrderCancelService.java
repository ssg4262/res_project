...
public class OrderCancelService {
	...
	public boolean orderCancel(int meal_order_num) {
		...
		int updateOrderCancelCount = orderDAO.updateOrderCancel(meal_order_num);

		/*-(insert,update,delete) 성공하면 commit 
		 *  실패하면 rollback(select 제외)---*/
		boolean isOrderCancelResult = false;

		if (updateOrderCancelCount > 0) {
			isOrderCancelResult = true;
			commit(con);
		} else {
			rollback(con);
		}
