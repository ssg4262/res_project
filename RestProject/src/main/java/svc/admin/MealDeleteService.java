...
public class MealDeleteService {
	// 멤버변수
	// 기본생성자
	// 메서드
	
	public boolean deleteMeal(String meal_id) {
		...
		int deleteMealCount = mealKitDAO.deleteMeal(meal_id);
		
		/*-(insert,update,delete) 성공하면 commit 
		 *  실패하면 rollback(select 제외)---*/
		boolean isMealDeleteResult = false;
		
		if(deleteMealCount > 0) {
			isMealDeleteResult = true;
			commit(con);
		} else {
			rollback(con);
		}
